...
public class AxonSimulator {
	public static void main(String[] args) {
		int numNodes = 10;
		double dt = 0.01;
		double duration = 3.5*numNodes;
		double stimDuration = 0.5;
		double delay = 0.1;
		Axon axon = new Axon(numNodes, dt);

		double intStimAmplitude = 20;
		double extStimAmplitude = 0;
		...
		SwingGraphics grapher = new SwingGraphics();
		...
		double scale = 11.5*numNodes;
		for (int i = 0; i < axon.numCompartments; i++) {
			double[] shiftedData = new double[N];
			for (int j = 0; j < N; j++) {
				shiftedData[j] = data[i][j] / scale + i;
			}
			grapher.graph(t, shiftedData, colors[i % colors.length]);
		}
...
class NodeCompartment {
	...
	public void calculate_dv(double intStimulus, double extStimulus) {
		...
		if (compartmentIndex == 0) {
			...
			double VeRight = axon.getNode(rightIndex).VeCoefficient * extStimulus; // Extracellular voltage of right
																					// node
